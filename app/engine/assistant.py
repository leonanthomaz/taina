    ...
    def run(self):
        while not self.should_stop:  # A loop continuará até a flag should_stop ser True
            if not self.greeting_done:  # Se ainda não fez a saudação
                ...
                # Aqui, vamos direto para a escuta após a saudação
                user_input = self.engine.engine_record_audio("Como posso ajudar?")  # Adicionando prompt aqui
                ...
                user_input = self.engine.engine_record_audio("Pode falar.")  # Mantendo o prompt para as iterações contínuas
                
                if user_input:
                    print(f"Entrada recebida: {user_input}")
                    # Responder ao usuário
                    self.engine.engine_response(user_input)
                    time.sleep(1)  # Pequena pausa antes de voltar a perguntar
